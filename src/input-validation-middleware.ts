...
if(!errors.isEmpty()) {
    ...
    const websiteUrlError = newErrors.find(err => err.field === 'websiteUrl')
    const descriptionError = newErrors.find(err => err.field === 'description')
    ...
    descriptionError && selectedErrors.push(descriptionError)
